...
<main id="main" class="main">
        ...
        <tbody>
            ...
            <tr>
                <td class="controls-table">
                    <div class="simon">
                        <div class="botones">
                            <form id="update{{$scientificprogram->id}}" method="POST" action="{{ route('scientificprograms.update', $scientificprogram->id) }}">
                                @csrf
                                @method('PUT')
                                <button type="button" onclick="showModalUpdate('{{$scientificprogram->id}}')" class="custom-btn btn-1" data-scientificprogram-id="{{$scientificprogram->id}}" data-toggle="tooltip" data-placement="left" title="Editar">
                                    <i class="fa-regular fa-pen-to-square"></i>
                                </button>
                            </form>

                            <form id="status{{$scientificprogram->id}}" method="POST" action="{{ route('scientificprograms.status', $scientificprogram->id) }}">
                                @csrf
                                @method('PUT')

                                <button type="button" onclick="updateStatus('{{$scientificprogram->id}}')" class="custom-btn {{($scientificprogram->status == 1) ? 'btn-2' : 'btn-3'}}" data-toggle="tooltip" data-placement="left" title="{{($scientificprogram->status == 1) ? 'Desactivar' : 'Activar'}}">
                        ...
                        <div class="botones3">
                            <form id="delete{{$scientificprogram->id}}" method="POST" action="{{route('scientificprograms.delete',$scientificprogram->id)}}">
                                @csrf
                                @method('DELETE')
            ...
            @endforeach
            {{-- <div class="pagination-scientificprogram">--}}
            {{-- {{ $dates->links() }}--}}
            {{-- </div>--}}
            @else
            <tr>
        ...
        <div class="modal-dialog modal-md modal-dialog-centered" style="max-width: 700px; top: 22%">
            <div style="height: 350px; border: none;" class="modal-content">
                <div style="display: flex; align-items: center; padding: 0; border: none; flex-direction: column;" class="modal-header">
                    ...
                    <form id="register_form" method="POST" action="{{ route('scientificprograms.store') }}" autocomplete="off" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
                                    <div class="mb-3 input-ecu">
                                        ...
                                        <input type="text" class="form-control input-skew" name="name_program" placeholder="Ingrese el ombre del Programa" maxlength="50" minlength="10" value="{{ old('name_program') }}" @if ($errors->has('name_program')) autofocus @endif required>
    ...
    <div style="overflow: hidden; height: auto; margin-top: -1%" class="modal fade" id="modal-update-{{$scientificprogram->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-dialog modal-md modal-dialog-centered" style="max-width: 750px; top: 22%">
            <div style="height: 300px; border: none;" class="modal-content">
                <div style="display: flex; align-items: center; padding: 0; border: none; flex-direction: column; margin-top: -1%" class="modal-header">
                    ...
                    <form id="update_form" method="POST" action="{{ route('scientificprograms.update', $scientificprogram->id) }}" autocomplete="off" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
                                    <div class="mb-3 input-ecu">
                                        <label class="form-label required">Nombre del Programa</label>
                                        <input type="text" class="form-control input-skew" name="name_program" placeholder="Ingrese el ombre del Programa" maxlength="50" minlength="10" value="{{ old('name_program', $scientificprogram->name_program) }}" @if ($errors->has('name_program')) autofocus @endif required>
                                        @if ($errors->has('name_program'))
                                        <div class="error-message">{{ $errors->first('name_program') }}</div>
                                        @endif
                                    </div>
                                ...
                                <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
                                    <div class="mb-3 input-ecu">
                                        <label class="form-label required">Fecha de Presentación</label>
                                        <input type="text" class="form-control input-skew" name="date_presentation" placeholder="Ingrese fecha de Presentación" maxlength="50" minlength="10" value="{{ old('date_presentation', $scientificprogram->date_presentation) }}" @if ($errors->has('date_presentation')) autofocus @endif required>
                                        @if ($errors->has('date_presentation'))
                                        <div class="error-message">{{ $errors->first('date_presentation') }}</div>
                                        @endif
                                    </div>
                                ...
                                <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
                                    <div class="mb-3 input-ecu">
                                        <label class="form-label required">Hora de Presentación</label>
                                        <input type="text" class="form-control input-skew" name="hour_presentation" placeholder="Ingrese hora de Presentación" maxlength="300" minlength="10" value="{{ old('hour_presentation', $scientificprogram->hour_presentation) }}" @if ($errors->has('hour_presentation')) autofocus @endif required>
                                        @if ($errors->has('hour_presentation'))
                                        <div class="error-message">{{ $errors->first('hour_presentation') }}</div>
                                        @endif
                                    </div>
                                ...
                                <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
                                    <div class="mb-3 input-ecu">
                                        ...
                                        <textarea type="text" class="form-control input-skew" name="activity" placeholder="Ingrese la actividad de la conferencia a presentar" maxlength="1100" minlength="10" @if ($errors->has('activity')) autofocus
                                                  @endif required>{{$scientificprogram->activity}}</textarea>
                                        @if ($errors->has('activity'))
                                        <div class="error-message">{{ $errors->first('activity') }}</div>
                                        @endif
                                    </div>
