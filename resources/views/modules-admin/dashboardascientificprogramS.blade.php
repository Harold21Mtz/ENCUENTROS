                ...
                @foreach($scientificprogramsS as $scientificprogramS)
                    <tr>
                        <td class="controls-table">
                            <div class="simon">
                                <div class="botones">
                                    <form id="update{{$scientificprogramS->id}}" method="POST"
                                          action="{{ route('scientificprogramsS.update', $scientificprogramS->id) }}">
                                        @csrf
                                        @method('PUT')
                                        <button type="button" onclick="showModalUpdate('{{$scientificprogramS->id}}')"
                                                class="custom-btn btn-1"
                                                data-scientificprogramS-id="{{$scientificprogramS->id}}"
                                                data-toggle="tooltip" data-placement="left" title="Editar">
                                            <i class="fa-regular fa-pen-to-square"></i>
                                        </button>
                                    </form>

                                    <form id="status{{$scientificprogramS->id}}" method="POST"
                                          action="{{ route('scientificprogramsS.status', $scientificprogramS->id) }}">
                                        @csrf
                                        @method('PUT')

                                        <button type="button" onclick="updateStatus('{{$scientificprogramS->id}}')"
                                                class="custom-btn {{($scientificprogramS->status == 1) ? 'btn-2' : 'btn-3'}}"
                                                data-toggle="tooltip" data-placement="left"
                                                title="{{($scientificprogramS->status == 1) ? 'Desactivar' : 'Activar'}}">
                                ...
                                <div class="botones3">
                                    <form id="delete{{$scientificprogramS->id}}" method="POST"
                                          action="{{route('scientificprogramsS.delete',$scientificprogramS->id)}}">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="modal-dialog modal-md modal-dialog-centered" style="max-width: 700px; top: 22%">
                <div style="height: 400px; border: none;" class="modal-content">
                    ...
                    <span style="font-size: 26px; padding-left: 16px" class="modal-title" id="exampleModalLabel"> <i
                        ...
                        <form id="register_form" method="POST" action="{{ route('scientificprogramsS.store') }}"
        ...
        @if(count($scientificprogramsS) > 0)
            @foreach($scientificprogramsS as $scientificprogramS)
                ...
                <div style="overflow: hidden; height: auto; margin-top: -1%" class="modal fade"
                     id="modal-update-{{$scientificprogramS->id}}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
                                            <div class="mb-3 input-ecu">
                                                ...
                                                <input type="text" class="form-control input-skew" name="name_program"
                                                       ...
                                                       minlength="10"
                                                       value="{{ old('name_program', $scientificprogramS->name_program) }}"
                                                       @if ($errors->has('name_program')) autofocus @endif required>
                                                @if ($errors->has('name_program'))
                                                    <div
                                        ...
                                        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
                                            <div class="mb-3 input-ecu">
                                                ...
                                                <input type="text" class="form-control input-skew"
                                                       ...
                                                       placeholder="Ingrese fecha de Presentación" maxlength="50"
                                                       minlength="10"
                                                       value="{{ old('date_presentation', $scientificprogramS->date_presentation) }}"
                                        ...
                                        <div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
                                            <div class="mb-3 input-ecu">
                                                ...
                                                <textarea type="text" class="form-control input-skew" name="activity"
                                                          ...
                                                          @if ($errors->has('activity')) autofocus
                                                          @endif required
                                                          style="min-height: 185px; max-height: 185px">{{$scientificprogramS->activity}}</textarea>
                                                @if ($errors->has('activity'))
                                                    <div class="error-message">{{ $errors->first('activity') }}</div>
                                                @endif
                                            </div>
